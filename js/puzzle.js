...
var background = document.getElementById("puzzleOuter");
var context = background.getContext('2d');

var backgroundWidth = 500; // 背景宽度
var padding = 10; // 图片边距
var column = 3; // 列数
var randomTitle = ''; //哪一組圖片
...
var start = function(){
    ...
    drawAllImage();
    if (counting) {
        // alert(Math.floor((Date.now() - startTime)/1000));
        clearInterval(intervalId);
        startTime = Date.now();
        intervalId = setInterval(updateCounter, 30);
    } else {
        startTime = Date.now();
        intervalId = setInterval(updateCounter, 30);
    }
    ...
    counting = true;
    btn.textContent = counting ? '重來' : '開始';
}
...
background.onclick = function(e) {
    ...
    if (checkIfFinish()) {
        var result = Math.floor((Date.now() - startTime)/1000);
        drawImageItem(imageIndexForPosition[lastIndex()], lastIndex());
        clearInterval(intervalId);
        setTimeout(() => {
            alert('嗨你花了' + result + '秒鐘完成拼圖');
        }, 500);
        isFinish = true;
    }
};
...
// 從3組圖裡選一組
var randomTitleF = function (){
    var ramdomIndex = function (max){
        return Math.floor(Math.random() * Math.floor(max));
    };
    var titleList = ['dog', 'sushi', 'nero'];
    randomTitle = titleList[ramdomIndex(3)];
}

// 绘制一个图片，index图片索引，position图片位置（0 ～ column^2-1）
var drawImageItem = function(index, position) {
    var img = new Image();
    img.src = './img/' + randomTitle + '_0' + String(index+1) + '.jpg';
    img.onload = () => {
